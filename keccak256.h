...
std::vector<uint8_t> keccak256(const std::vector<uint8_t>& data) {
    keccak_state* state = nullptr;
    if (keccak_init(&state, 64, 24) != 0) {
        throw std::runtime_error("Keccak initialization failed");
    }
    
    if (keccak_absorb(state, data.data(), data.size()) != 0) {
        keccak_destroy(state);
        throw std::runtime_error("Keccak absorb failed");
    }

    std::vector<uint8_t> digest(32);
    if (keccak_digest(state, digest.data(), digest.size(), 0x01) != 0) {
        keccak_destroy(state);
        throw std::runtime_error("Keccak digest failed");
    }
